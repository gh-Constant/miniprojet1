...
// 1. Initialiser l'authentification Google
router.get('/google',
  passport.authenticate('google', {
    scope: ['profile', 'email'],
    session: false // IMPORTANT : stateless
  })
);
...
router.get('/google/callback',
  ...
  (req, res) => {
    // Si on arrive ici, l'authentification a réussi et req.user contient l'utilisateur
    try {
      // Générer le token JWT
      const token = generateToken(req.user._id);
      
      // Rediriger vers le frontend avec le token en paramètre d'URL
...
router.get('/discord/callback',
  ...
  (req, res) => {
    try {
      ...
      console.error('Erreur token Discord:', error);
      res.redirect(`${process.env.FRONTEND_URL}/login?error=token_generation_failed`);
    }
  }
);
...
router.get('/github/callback',
  ...
  (req, res) => {
    try {
      ...
      console.error('Erreur token GitHub:', error);
      res.redirect(`${process.env.FRONTEND_URL}/login?error=token_generation_failed`);
    }
  }
);
